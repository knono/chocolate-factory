...
# Instalar Tailscale versión 1.92.0+ (App Capabilities support for Sprint 18)
...
RUN wget -O /tmp/tailscale.tgz https://pkgs.tailscale.com/stable/tailscale_1.92.0_amd64.tgz && \
    ...
    cp /tmp/tailscale_1.92.0_amd64/tailscale /usr/local/bin/ && \
    cp /tmp/tailscale_1.92.0_amd64/tailscaled /usr/local/bin/ && \
    chmod +x /usr/local/bin/tailscale /usr/local/bin/tailscaled && \
    rm -rf /tmp/tailscale.tgz /tmp/tailscale_1.92.0_amd64
