# src/services/production_service_optimized.py
"""Optimized production service with caching."""
...
from src.repositories.production_repository import ProductionRepository
from src.ml.serving.model_registry import model_registry
from src.core.config import settings
from src.core.cache.cache_manager import cache_key_wrapper, cache_manager

logger = logging.getLogger(__name__)


class ProductionService:
    """Service for managing production business logic with caching."""
    ...
    @cache_key_wrapper(prefix="production:recent", ttl=30)  # Cache for 30 seconds
    ...
    @cache_key_wrapper(prefix="quality:metrics", ttl=10)  # Cache for 10 seconds
    ...
    async def create_production_batch(self, batch_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cache_manager.invalidate_pattern("production:recent")
        cache_manager.invalidate_pattern("quality:metrics")
        cache_manager.invalidate_pattern("production:stats")
    ...
    @cache_key_wrapper(prefix="production:stats", ttl=60)  # Cache for 1 minute
    ...
    @cache_key_wrapper(prefix="batch:analysis", ttl=120)  # Cache for 2 minutes
