...
execute_backfill() {
    ...
    case $mode in
        ...
        "full")
            ...
            ;;
        "ree")
            endpoint="/gaps/backfill?days_back=7"
            description="Backfill datos REE y Weather"
            print_warning "Nota: endpoint /gaps/backfill procesa ambos tipos de datos"
            ;;
        "weather")
            endpoint="/gaps/backfill?days_back=$DAYS_BACK"
            description="Backfill datos REE y Weather"
            print_warning "Nota: endpoint /gaps/backfill procesa ambos tipos de datos"
            ;;
        *)
